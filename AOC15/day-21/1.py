    ...
    for weapon in possible_weapons:
        for armor in possible_armors:
            for ring in possible_rings:
                ...
                if helper([100, player_damage, player_armor], [i for i in boss]): min_cost = min(min_cost, cost)
    
    return min_cost
    
with open(input_file) as f:
    part1_ip, boss = f.read().split("\n\n\n")
    weapons, armors, rings = part1_ip.split("\n\n")
    boss = boss.split("\n")
    print ("\nPart - 1: Main", part_1(boss, weapons, armors, rings))